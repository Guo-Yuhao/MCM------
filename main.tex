...
\begin{enumerate}[\bfseries 1.]
	\item \textbf{Model construction.} We develop a suite of mathematical models to analyze and optimize the lunar material delivery system. A static baseline is first used to compare three transportation scenarios, followed by a dynamic extension in which the transportation ratio $\alpha_i$ evolves over time to reflect technological progress and cost reduction, with stochastic terms added to capture uncertainty.
	\item \textbf{Extended tasks.} Additional modules are integrated into the core framework. A water demand model estimates annual net water requirements under varying recycling efficiencies, while an environmental impact model evaluates rocket-related $CO_2$ and black carbon emissions and Space Elevator energy consumption, enabling joint consideration of cost, time, and sustainability.
	\item \textbf{Model solving and evaluation.} The nonlinear multi-objective problem is solved using a Stage-wise Extended Search Algorithm (SESA) combined with a Greedy Marginal Cost Method (GMCM). Pareto-front analysis and knee-point identification determine optimal trade-offs, and sensitivity and robustness analyses confirm model reliability, showing the dynamic hybrid strategy to be superior.
\end{enumerate}
%===========================第二部分==模型准备==========================================================
