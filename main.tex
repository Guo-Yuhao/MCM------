...
Some people envision a Space Elevator System, powered by electricity, offering a scalable
...
Based on the plan above,The Moon Colony Management (MCM) Agency is preparing to build a Moon Colony with an
...
\begin{itemize}
	...
	\item \textit{\textbf{Assumption 3:}}水资源消耗：人均日用水量0.15吨，年人均54.75吨，循环效率 $\gamma=0.85$。
	\\$\hookrightarrow$ \textit{\textbf{Explanation:}}参考国际空间站用水标准，循环效率符合2050年技术水平预测。

	\item \textit{\textbf{Assumption 4:}}非完美状态参数：SE缆绳摆动临界角5°，年故障概率基准值2\%；TR发射失败概率3\%，发射窗口关闭导致运力折损30\%。
	\\$\hookrightarrow$ \textit{\textbf{Explanation:}}基于航天工程历史数据与题目场景合理设定。

	\item \textit{\textbf{Assumption 5:}}成本参数：SE固定成本1000亿美元，单位可变成本1000美元/吨；TR单次发射成本1亿美元。
	\\$\hookrightarrow$ \textit{\textbf{Explanation:}}参考Falcon Heavy当前成本与未来技术迭代趋势，量级合理。

	\item \textit{\textbf{Assumption 6:}}顶端火箭发射失败时成本不计。
	\\$\hookrightarrow$ \textit{\textbf{Explanation:}}由于顶端火箭载荷量未知，成本波动计入原成本内。

	\item \textit{\textbf{Assumption 7:}}月球人均用水量近似为地球人均用水量。
	\\$\hookrightarrow$ \textit{\textbf{Explanation:}}简化问题，方便计算。
...
随着时间的推移和技术的发展，太空电梯系统的单位可变成本$c_{var}^{SE}$逐渐下降。采用如下理想化模型表示这一过程：
...
\subsection{火箭发射失败的修正}

\subsubsection{失败概率与有效载荷}
...
其中$P_{\mathrm{fail}}(i)$可以是固定值，也可随时间或发射条件变化。

\subsubsection{修正成本公式}

成本公式中，传统火箭的单位成本应修改为：

\begin{equation}
	c_{TR}(i) = c_{\mathrm{launch}} + P_{\mathrm{fail}}(i) \cdot (c_{\mathrm{launch}} + c_{\mathrm{cargo\_loss}})
\end{equation}

这样将发射失败的风险成本纳入总成本计算。

\subsection{系绳摇晃的科里奥利力与风切变扰动}

\subsubsection{瞬时可用度函数}
...
其中$\theta(t)$为系绳摆动角度，$\theta_{\mathrm{crit}}$为临界摆动角度（5$^\circ$）；$W_{\mathrm{wind}}(t)$为风速，$v_{\mathrm{safe}}$为安全风速限制。风场满足韦布尔分布。

\subsubsection{顶端火箭发射失败概率}

由于太空电梯顶端仍需火箭发射将货物送入轨道，存在失败概率$P_{\mathrm{fail}}'$。

\subsubsection{修正后的年有效运力}

综合考虑摆动限制、维护停机和火箭发射失败，太空电梯的修正后年有效运力为：

\begin{equation}
	\kappa'_{SE} = \kappa_{SE} \cdot \frac{1}{T_{\mathrm{year}}} \int_{0}^{T_{\mathrm{year}}} \eta_{SE}(t) \, dt \cdot (1 - \beta_{\mathrm{maint}}) \cdot (1-P_{\mathrm{fail}'})
\end{equation}

其中：
\begin{itemize}
	\item $T_{\mathrm{year}}$为一年时长
	\item $\frac{1}{T_{\mathrm{year}}} \int_{0}^{T_{\mathrm{year}}} \eta_{SE}(t) \, dt$为年可用度因子
	\item $\beta_{\mathrm{maint}}$为维护停机系数
	\item $(1-P_{\mathrm{fail}'})$为火箭发射成功率
\end{itemize}
...
\begin{itemize}
	...
	\item $\gamma=0$为水资源循环效率
	\item $P_{pop}=10$万人为月球殖民地人口
	\item $w_{per}=0$吨/(人·天)为人均日用水量
\end{itemize}

\subsubsection{额外运输成本}

一年的净用水需求所产生的额外运输成本取决于选用的运输方式。在此视角下，不同策略的成本如下：

对于仅使用太空电梯的情况：

\begin{equation}
	C_{water,SE} = M_{net} \cdot c_{var}^{SE}
\end{equation}

对于仅使用传统火箭的情况：

\begin{equation}
	C_{water,TR} = \frac{M_{net}}{m_{load}} \cdot c_{launch}
\end{equation}

对于静态混合策略，成本为：

\begin{equation}
	C_{water,static} = \alpha \cdot M_{net} \cdot c_{var}^{SE} + (1-\alpha) \cdot \frac{M_{net}}{m_{load}} \cdot c_{launch}
\end{equation}

其中$\alpha$为太空电梯的运输比例。

在动态模型中，工期尾部阶段的$\alpha_i$取值基本为1，其对应的成本为：

\begin{equation}
	C_{water,dynamic} = M_{net} \cdot c_{var}^{SE}
\end{equation}

这反映了在工期后期，当太空电梯成本优势明显时，倾向于优先采用太空电梯运输水资源的策略。

\subsection{环境污染模型}

\subsubsection{原有各模型的环境影响因子}

传统火箭系统（TR-only模式）的主要环境污染源自火箭发射产生的二氧化碳排放以及黑碳和氧化铝对臭氧层的破坏。这些排放量与火箭发射次数成正比，其环境影响系数为：

\begin{equation}
E_{TR-only} = \frac{M_{tot}}{m_{load}} \cdot (\omega_{CO_{2}}\cdot \epsilon_{CO_{2}}+\omega_{BC}\cdot \epsilon_{BC}+\omega_{Al_2O_3}\cdot \epsilon_{Al_2O_3})
\end{equation}

其中：
\begin{itemize}
	\item $\omega_{CO_{2}}$、$\omega_{BC}$、$\omega_{Al_2O_3}$分别为单次发射中各污染物的排放量
	\item $\epsilon_{CO_{2}}$、$\epsilon_{BC}$、$\epsilon_{Al_2O_3}$分别为各污染物的环境影响权重
\end{itemize}

太空电梯系统（SE-only模式）的主要环境污染源自运送货物时耗费的电力。这些电力可能来自于非清洁能源，在生产电力的过程中造成污染。其环境影响系数为：

\begin{equation}
E_{SE-only} = M_{tot} \cdot e_{per}\cdot (1-\eta_{clean})
\end{equation}

其中：
\begin{itemize}
	\item $e_{per}$为单位货物运输的能源消耗环境影响系数
	\item $\eta_{clean}$为清洁能源的使用率
\end{itemize}

静态混合模式的环境污染同时来自于两种污染源，其环境影响系数为：

\begin{equation}
E_{static} = \frac{(1-\alpha) \cdot M_{tot}}{m_{load}} \cdot (\omega_{CO_{2}}\cdot \epsilon_{CO_{2}}+\omega_{BC}\cdot \epsilon_{BC}+\omega_{Al_2O_3}\cdot \epsilon_{Al_2O_3}) + \alpha \cdot M_{tot} \cdot e_{per}\cdot (1-\eta_{clean})
\end{equation}

其中$\alpha$为太空电梯的运输比例。

\subsubsection{动态模型中的环境影响}

动态模型中，各阶段的环境影响系数取决于该阶段的具体运输方式分配。定义第$i$阶段的环境影响为：

\begin{equation}
E_{dynamic}(i) = \frac{Q_{TR}(i) - Q_{TR}(i-1)}{m_{load}} \cdot (\omega_{CO_{2}}\cdot \epsilon_{CO_{2}}+\omega_{BC}\cdot \epsilon_{BC}+\omega_{Al_2O_3}\cdot \epsilon_{Al_2O_3}) + (Q_{SE}(i) - Q_{SE}(i-1)) \cdot e_{per}\cdot (1-\eta_{clean})
\end{equation}

其中：
\begin{itemize}
	\item $Q_{SE}(i)$为截止第$i$阶段的太空电梯累积运输量
	\item $Q_{TR}(i)$为截止第$i$阶段的传统火箭累积运输量
\end{itemize}

全生命周期的总环境影响为：

\begin{equation}
E_{total} = \sum_{i=1}^{k} E_{dynamic}(i)
\end{equation}

其中$k$为所需的总阶段数。

\subsubsection{环境影响最小化策略}

为了最小化环保影响，应优先采用以下措施：

\begin{enumerate}
	\item \textbf{增加清洁能源使用率}：通过扩大太空电梯的清洁能源供电比例$\eta_{clean}$，可有效降低SE系统的环境污染。
	\item \textbf{优化发射场选择}：优先选择低纬度发射场以减少火箭升空过程中对臭氧层的破坏。
	\item \textbf{改进火箭技术}：采用更环保的火箭燃料和推进系统，减少单位发射的污染物排放。
	\item \textbf{合理规划运输比例}：在满足工期和成本约束的前提下，根据环境影响系数动态调整SE和TR的运输比例。
\end{enumerate}

\subsubsection{环保-成本-工期的多目标平衡}

将环境影响纳入优化目标后，混合策略和动态模型的最优$\alpha_i$可能需要调整。通过帕累托前沿分析，可识别在环境污染、运输成本和建设工期之间达到最佳平衡的方案。

定义综合目标函数为：

\begin{equation}
F = w_1 \cdot f_{cost} + w_2 \cdot f_{time} + w_3 \cdot E_{total}
\end{equation}

其中$w_1$、$w_2$、$w_3$分别为成本、工期和环境影响的权重系数，满足$w_1+w_2+w_3=1$。
