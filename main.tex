...
运输分配比例$\alpha(t)$由太空电梯和传统火箭系统的瞬时单位成本比$\rho(t)$驱动：

\begin{equation}
	\rho(t)=\frac{c_{var}^{SE}(t, Q_{SE}(t))}{c_{TR}(t, Q_{TR}(t))}
\end{equation}

根据成本比的大小，对$\alpha_i$的决策规则为：

\begin{itemize}
	\item 当$\rho(t)<0.8$（SE更便宜）时：$\alpha_i=1$
	
	\item 当$\rho(t)>1.2$（TR更便宜）时：$\alpha_i=0$
	
	\item 当$0.8 \leq \rho(t) \leq 1.2$（成本均衡）时：$\alpha_i \in (0,1)$，为重点优化方向，目标是最小化区间内的成本和环境影响
\end{itemize}
...
\begin{itemize}
	...
	\item $\gamma=0.85$为水资源循环效率
	\item $P_{pop}=10$万人为月球殖民地人口
	\item $w_{per}=0.15$吨/(人·天)为人均日用水量
\end{itemize}

\subsubsection{额外运输成本}

一年的净用水需求所产生的额外运输成本取决于选用的运输方式。对于仅使用太空电梯的情况：
...
对于混合策略，需根据动态模型中的最优分配比例$\alpha$计算。
...
\newpage
%===============================================第四部分=============================================
>>>>>>> c50d6c0d6a24a51b76158cccc6273f7dd0119408
