...
\begin{enumerate}[\bfseries 1.]
	\item \textbf{Model construction.} We formulate mathematical models for lunar material delivery, comparing three scenarios with a static baseline and extending to a dynamic model where the transport ratio $\alpha_i$ evolves with technological progress, cost reduction, and stochastic uncertainty.
	\item \textbf{Extended tasks.} The framework incorporates a water demand model for annual net requirements under different recycling efficiencies and an environmental impact model accounting for rocket $CO_2$/black carbon emissions and Space Elevator energy use.
	\item \textbf{Model solving and evaluation.} The resulting nonlinear multi-objective problem is solved via SESA–GMCM, with Pareto-front and knee-point analyses identifying optimal trade-offs; sensitivity and robustness tests verify reliability and demonstrate the superiority of the dynamic hybrid strategy.
\end{enumerate}
%===========================第二部分==模型准备==========================================================
...
conditions, comparing different material transportation schemes.
\subsection{Scenario 1: SE-Only Mode}
%\subsubsection{Transportation Capacity under Perfect Conditions}
%太空电梯系统的年运输能力为：
The annual capacity of the Space Elevator (SE) is:\quad$\kappa_{SE,total}=179,000 \text{ tons/year}$.\\
The total duration required to transport all materials is:\quad$T_{SE}=\frac{M_{tot}}{\kappa_{SE,total}} \approx 558.7 \text{ years}$.\\
...
the Traditional Rocket (TR) system is:\quad$\kappa_{TR}=10 \cdot f_{TR}(j) \cdot m_{load}=30,000 \text{ tons/year}$.\\
...
%物资全部运输所需的总工期为：
The total duration required to transport all materials is:\quad$T_{TR}=\frac{M_{tot}}{\kappa_{TR}}=3333.3\text{ years}$.\\	
...
This algorithm finds dynamic capacity planning solutions by progressively extending time intervals, with core steps as follows:
\begin{itemize}
	\item \textbf{Cumulative capacity check}: At each iteration, aggregate the available capacity for the current phase. If total demand cannot be met, terminate the iteration.
	\item \textbf{Option set and cost ordering}: Consolidate capacity options and sort them in ascending order of cost, prioritizing lower-cost sources.
	\item \textbf{Greedy allocation}: Allocate resources sequentially starting from the lowest-cost option until demand is satisfied or capacity is exhausted.
	\item \textbf{Mixed-tech constraints}: If minimum deployment requirements exist, reserve the minimum capacity first, then perform greedy allocation on the remaining demand.
	\item \textbf{Cost aggregation and logging}: Calculate costs for each phase, logging capacity allocations and total costs for subsequent feasibility screening and optimization.
\end{itemize}
...
\begin{figure}[htbp]
	\centering
	\begin{subfigure}[b]{0.95\textwidth} %设置缩放比例，这里的.5代表缩放为原来的50%
...
\textbf{Rocket failure probability fluctuations}: Modeled as a Beta distribution (biased towards the mean but with risk at extreme values).
\textbf{Cost fluctuations}: Assuming future prices are influenced by inflation and technological progress, costs follow a normal distribution.
\textbf{Technological improvement rate fluctuations}: Parameters in the $c_{var}^{SE}$ formula follow a normal distribution, affecting the decline rate of $c_{var}^{SE}$.
\paragraph{Robustness evaluation metrics}
\textbf{Objective function deviation}: Measures the degree of change in the objective function value after perturbation relative to the baseline value:\quad$R = \frac{|f_{\mathrm{perturbed}} - f_{\mathrm{baseline}}|}{f_{\mathrm{baseline}}}$.\quad
\textbf{Worst-case performance degradation}: Takes the worst 5\% of the sample results for risk assessment.
\paragraph{Analysis Method}
Use the original parameters to obtain the baseline duration $T_0$, baseline cost $f_0$, and baseline environmental impact $E_0$.
，and perform simulations with perturbations:
\begin{itemize}
...
Models performance is shown in Tables 3 and 4.
\begin{table}[hbtp]
  \centering
  \caption{results}
  \label{tab:results}
...
\begin{itemize}
	%1. 有灵敏度分析与稳健性分析
	\item The dynamic extended model introduces a time-adaptive parameter $\alpha$(i), enablingflexible adjustment of transport ratios, and using Pareto front analysis with knee-point detection, the model identifies balanced solutions between economic efficiency and sustainability.
...
\begin{itemize}
	...
	\item Combine the Stage‑wise Extended Search Algorithm with metaheuristics to improve convergence speed and optimize over higher‑dimensional objectives.
\end{itemize}
...
\backgroundsetup{
  ...
  contents={\includegraphics[width=\paperwidth,height=\paperheight]{img/background_letter.png}}, % 图片路径（你后续替换），铺满整页
...
\begin{subappendices}  % 附录环境开始
	...
	Query: <Briefly introduce the Knee point>

	Output: <The Knee point (elbow point) is the optimal trade-off point on the Pareto frontier, named for the elbow-like bend of the curve. Before this point, improving an indicator brings high marginal benefit at low cost; after it, the marginal cost surges sharply. It is the cost-effective optimal solution with the most practical decision-making value selected from Pareto optimal solutions in multi-objective optimization>
	
	2.OpenAI ChatGPT

	Query: <I will send you the Chinese paragraphs of my thesis, and please translate the content of these paragraphs into English. Requirements: standard language usage, no grammatical errors, no mistranslations or omissions>
	
	Output: <Okay, please paste the Chinese paragraphs of your thesis that need to be translated here. I will accurately translate them in accordance with the norms of academic English, ensuring no grammatical errors, mistranslations or omissions>

	3.Github Copilot

	Query: <During the data visualization process, I generated a scatter plot, Could you provide suggestions for improving the chart from the perspectives of structural design, visual presentation, and key information emphasis, in order to enhance clarity and academic quality?>
	
	Output: <The chart can be improved through the following aspects: 1.Structural Design Optimization, Adjust the axis ranges appropriately to avoid excessive blank space or data compression; 2.Visual Presentation Enhancement, Use a colorblind-friendly and high-contrast color palette, avoiding excessive use of colors;>

	4.Google Gemini

	Query: <I am trying to find authoritative information on daily per capita water consumption for typical residential use.Specifically, I need data on the average amount of water used per person per day under normal living conditions.>
	
	Output: <The average daily per capita residential water consumption can vary by country and region. Based on available data: Average residential water use is approximately  0.35 tons per person per day, including indoor and outdoor use.>
