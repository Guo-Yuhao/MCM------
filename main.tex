...
\begin{equation}
	\kappa_{TR}=10 \cdot f_{TR}(j) \cdot m_{load}=10 \times 20 \times 150=30,000 \text{ 吨/年}
\end{equation}
...
物资全部运输所需的总工期为：
\begin{equation}
	T_{TR}=\frac{M_{tot}}{\kappa_{TR}}=\frac{1 \times 10^8}{30000}=3333\text{ 年}
\end{equation}
...
\begin{itemize}
	...
	\item \textbf{工期最优模式}（$\alpha^* \approx 0.499$）：采用最优分配比例，工期约为$\max\left(\frac{0.417 \times 10^8}{179000}, \frac{0.583 \times 10^8}{25000}\right) \approx 232$年，在基础设施固定成本和按次发射成本间实现平衡。
	
	\item \textbf{平衡最优选择}：三种方案的优劣取决于对工期最小化或成本最小化的优先级权重。
\end{itemize}
