...
In its mature configuration, the system comprises three Galactic Harbours spaced 120° apart along the equator. Each harbour includes one Earth port connected to two 100,000-km tethers leading to apex anchors. Operating cooperatively, multiple space elevators can routinely transport large payloads from Earth to geosynchronous orbit (GEO) and onward to the apex anchors, where they are transferred to rockets for onward delivery with substantially reduced propellant requirements.
%% 这是问题背景的概念图
\begin{figure}[htbp]
	\centering
...
In the literature\upcite{ishimatsu2016generalized}, a generalized multi-commodity network flow (GMCNF) model has been proposed to optimize logistics within the Earth–Moon–Mars system.
 ...
 total mass launched from Earth (TLMLEO) when combined with$LOX/LH_2$propulsion and aerocapture2. Sensitivity analysis shows lunar ISRU becomes economically viable above 1.8 kg/year/kg productivity, 
...
Carbon dioxide ($CO_{2}$) emissions from rockets contribute negligibly to RF compared to particulate emissions.
Current global RF from rocket launches is estimated at 16±8$mWm^{-2}$, with BC accounting for 70 percents of this value.

\subsection{Our work}%-----------------------------------------------------------
We do such things as follows:
\begin{figure}[htbp]
	\centering
...
\begin{enumerate}[\bfseries 1.]
	\item \textbf{We do model construction.} We build a series of mathematical models to analyze and optimize the lunar material delivery system. Starting from a static baseline model, we compare three transportation scenarios—Space Elevator only, Traditional Rocket only, and a hybrid strategy. On this basis, we further establish a dynamic extended model that allows the transportation ratio $\alpha_i$ to evolve over time, incorporating technical progress and cost reduction. Finally, we introduce stochastic elements, such as rocket launch failure and elevator tether swaying, to ensure the model reflects realistic uncertainty.
	\item \textbf{We do extended tasks.} We integrate additional requirements into the core framework, including one-year water supply planning and environmental impact assessment. The water demand model quantifies the annual net water requirement under different recycling efficiencies, while the environmental impact model evaluates $CO_2$ and black carbon emissions from rockets, as well as energy use from the Space Elevator. This extension allows the model to balance cost, time, and ecological sustainability.
...
\begin{itemize}
	...
	\item \textit{\textbf{Assumption 2:}}$\alpha_i$ is a piecewise constant function: The total project duration is divided into 50 time intervals (each interval spanning 1–2 years), with $\alpha_i$ being a constant for each interval.
	\\$\hookrightarrow$ \textit{\textbf{Explanation:}}Balancing computational accuracy and efficiency, tailored to meet the requirements of MCM competition solutions, easy to implement with intuitive results.

	\item \textit{\textbf{Assumption 3:}}Water consumption: Daily per capita water usage of 0.35 tons, with a water recycling efficiency of $\gamma=0.95$.
	\\$\hookrightarrow$ \textit{\textbf{Explanation:}}Referencing the water usage standards of the International Space Station, the recycling efficiency aligns with projected technological levels for 2050.

	\item \textit{\textbf{Assumption 4:}}The space elevator's clean energy utilization rate is 0.3, and it will steadily increase with technological advancements until it reaches 100%.
	\\$\hookrightarrow$ \textit{\textbf{Explanation:}}Based on green science principles and reasonable scenario settings.

	\item \textit{\textbf{Assumption 5:}}Imperfect state parameters: TR transmission failure probability of 1%, which will decrease to 0.1% with technological advancement.
	...
	\item \textit{\textbf{Assumption 6:}}Cost Parameters: SE fixed cost: \$6.2 billion; Unit variable cost: \$250,000 per ton; TR single launch cost: \$67 million.
	\\$\hookrightarrow$ \textit{\textbf{Explanation:}}Considering Falcon Heavy's current cost and future technological iteration trends, the scale is reasonable.

	\item \textit{\textbf{Assumption 7:}}The cost of an apex rocket launch failure is not counted.
	...
	\item \textit{\textbf{Assumption 8:}}The original rocket launch center had zero startup costs.
	\\$\hookrightarrow$ \textit{\textbf{Explanation:}}Given the existing launch site's sufficiently large daily throughput capacity, there should be no cost associated with its activation.
\end{itemize}
...
The total duration required to transport all materials is:
\begin{equation}
	T_{TR}=\frac{M_{tot}}{\kappa_{TR}}=\frac{1 \times 10^8}{30000}=3333\text{ years}
\end{equation}
...
\begin{figure}[htbp]
	\centering
	\begin{subfigure}[b]{0.75\textwidth} %设置缩放比例，这里的.5代表缩放为原来的50%
...
%===========================================第四部分====================================================================
\section[Model 2: Dynamic Extended Model]{Model 2: Dynamic Extended Model (Time-Adaptive Strategy)}
...
\begin{enumerate}
	...
	development to fully leverage the opportunities brought by cost reduction.%根据技术发展动态调整运输方式比例，充分利用成本下降机遇
	...
	decision-making for each interval, facilitating implementation and adjustment.%将总工期划分为50个区间，每个区间独立决策，便于实施和调整
...
where $c_{\mathrm{cargo\_loss}}$ is the cargo loss cost. Typically, a re-launch is provided at no additional cost to the customer in the event of a failure, so the formula can be simplified as:
...
In the cost formula, the unit cost of TR $c_{TR}(i)$ should be revised to:
\begin{equation}
...
Conventional ground-based tethers can be fitted with $k$ approximately 0.019, 
which does not match the current tether material and high-altitude environment. 
The current design is for strong constraint/high tension/high wind resistance. 
$\theta_{\mathrm{crit}}$ can be determined as $0.1\ \mathrm{rad}$ and $v_{\mathrm{safe}}$ 
as $15\ \mathrm{m/s}$ based on altitude. Furthermore, the current $k$ is back-calculated 
as $4.44\times10^{-4}$. This means that when $v_{wind}(t) = v_{\mathrm{safe}}$, 
$\theta(t) \approx \theta_{\mathrm{crit}}$. This can be combined into a single threshold effect.
...
abstracting the wind environment into two categories of states:

\begin{itemize}
	\item \textbf{Normal State}：Availability fluctuates slowly around a low-frequency baseline.
	\item \textbf{Extreme State}：Represents long-term storm eras or abnormal circulation periods, 
	with significantly reduced availability lasting for several intervals.
\end{itemize}
...
\begin{figure}[htbp]
	\centering
	\begin{subfigure}[b]{.6\textwidth} %设置缩放比例，这里的.5代表缩放为原来的50%
		\includegraphics[width=\textwidth]{img/Figure_462.png}
		\caption*{}
		\label{}
	\end{subfigure}
	\caption{462}\label{subfigure}
\end{figure}
...
The main environmental pollution of the SE-only model comes from the electricity consumed during cargo transportation, 
...
The environmental impact coefficient of the dynamic model is similar to the above formula:
\begin{equation}
\begin{split}
...
\begin{enumerate}
	...
	\item \textbf{Mixed-technology constraints}: If minimum deployment levels for SE and TR are required, reserve those minima first, then apply the greedy allocation to remaining demand.
...
As shown in Figure 10, the visualization of the impact of the dynamic $\alpha$ strategy on sensitivity analysis after applying multi-factor analysis is presented. Figure 11 shows the performance comparison of different strategies under sensitivity analysis.
...
\end{itemize}
\subsubsection{Robustness evaluation metrics}
\begin{itemize}
